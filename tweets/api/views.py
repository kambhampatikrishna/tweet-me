...
from .serializers import TweetModelSerialiazer
...
class TweetLikeAPIView(APIView):
    ...
    def get(self,request,pk,format=None):
        ...
        if request.user.is_authenticated:
            is_liked=Tweet.objects.like_toogle(request.user,tweet_qs.first())
    ...
    def get(self,request,pk,format=None):
        ...
        if tweet_qs.exists() and tweet_qs.count()==1:
            ...
            if new_tweet is not None:    
                data=TweetModelSerialiazer(new_tweet).data
                return Response(data)
            message="cannot retweet the same in 1 day"      
        return Response({"message":message},status=400)

class TweetCreateAPIView(generics.CreateAPIView):
    serializer_class=TweetModelSerialiazer
    permission_classes=[permissions.IsAuthenticated]
...
class TweetListAPIView(generics.ListAPIView):
    ...
    def get_queryset(self,*args,**kwargs):
        ...
        return qs
    # def get_serializer_context(self,*args,**kwargs):
    #     context=super(TweetListAPIView,self).get_serializer_context("request")
    #     context['request']=self.request
    #     return context